...
func desugaringExercise(user: User, appTheme: Style) -> [String : () throws -> Color?] {
    [
        ...
        "Step 2: Remove ?. (optional chaining)": {
            ...
            let y : Color?
            if let yUnwrapped = user.avatar {
                y = yUnwrapped.style.backgroundColor
            } else {
                y = nil
            }
            let x : Color?
        ...
        "Step 6: Verify desugaring using FakeOptional": { try { () -> FakeOptional<Color> in  // 🚨 DO NOT MODIFY THIS LINE! It ensures that you are returning a FakeOptional.
            ...
            let y : FakeOptional<Color>
            switch user.avatar.fakeOptional {
                case FakeOptional.some(let yUnwrapped):
                    y = yUnwrapped.style.backgroundColor.fakeOptional
                case FakeOptional.none:
                    y = FakeOptional.none
            }
            let x : FakeOptional<Color>
            switch y {
                case FakeOptional.some(let unwrapped):
                x = FakeOptional.some(unwrapped)
                case FakeOptional.none:
